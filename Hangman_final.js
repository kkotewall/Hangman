...
// math function to select word bank string and save to variable; console check confirmed
	solution = wordBank[Math.floor(Math.random() * wordBank.length)];


// count the string length of the selected word variable; console check confirmed
	answer = solution.replace(/([A-Z])/g, " _ ")

// event get key down and save to correct variables; console check confirmed
	document.onkeyup = function(event) {
		var userGuess = event.key;
		console.log(userGuess.toUpperCase())};

// compare if varaible guessed is in alphabet array; console check confirmed
...
// empty array for position of correctly guessed letters
	...
	for(var i=0; i<solution.length;i++) {
    	...
    	// IF NOT in word selected variable add to function incorrectly guessed letters
    		else (push(userGuess).voidLetters[])
    			guess.countb er++
			alert("Sorry, please select a different letter.")
	}

	// loop through index of positions, replacing userGuess in answer by creating new answer string
